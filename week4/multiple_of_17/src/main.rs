...
fn main() {
    ...
    let five = BigInt::from(5 as u64);
    let seventeen = BigInt::from(17 as u64);
    let m_seventeen = BigInt::from(17 as i64);

    for line in lock.lines() {
        let line = line.unwrap();

        let mut number: BigInt = line.parse().unwrap();
        ...
        let lsd = BigInt::from(number.digits[0] as u64);

        let mut dif = (number >> (1 as u8)) - (&five * &lsd);

        while dif > seventeen {
            let lsd = BigInt::from(dif.digits[0] as u64);

            dif = (&dif >> (1 as u8)) - (BigInt::from(5 as u64) * lsd);
        }

        if dif == seventeen || dif == m_seventeen || dif == zero {
            println!("1");
        } else {
            println!("0");
        }
