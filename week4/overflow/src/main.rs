...
fn main() {
    ...
    let max = BigInt::i32_max();

    for line in lock.lines() {
        let line = line.unwrap();
        println!("{line}");

        let input: Vec<_> = line.split_ascii_whitespace().collect();

        let first: BigInt = input[0].parse().unwrap();
        let op = input[1];
        let second: BigInt = input[2].parse().unwrap();

        if first > max {
            println!("first number too big");
        }

        if second > max {
            println!("second number too big");
        }
